...
class Celeba(Dataset):
    ...
    def __init__(self, data_dir, atts, img_resize, batch_size, prefetch_batch=2, drop_remainder=True,
                 num_threads=16, shuffle=True, buffer_size=4096, repeat=-1, sess=None, part='train', crop=True, image_labels=None, file_name=None):
        ...
        if image_labels is not None:
            ...
            img_paths.append(os.path.join(img_dir_jpg,file_name))
        ...
        if part == 'test':
            ...
            img_paths = img_paths[(len(img_paths)-1):len(img_paths)]
            labels = labels[(len(labels)-1):len(labels)]
        else:
...
if __name__ == '__main__':
    ...
    batch = data.get_next()
    #print(len(data))
    x = True if len(data)==637 else False
    print("Length of dataset test: " + str(x))
    #print(batch[1][1], batch[1].dtype)
    y = True if len(batch[1][1])==13 else False
    y_type = True if batch[1].dtype=="int64" else False
    print("Length of batch test: " + str(y))
    print("Type of image batch test: " + str(y_type))
    #print(batch[0].min(), batch[1].max(), batch[0].dtype)
    z_min = True if batch[0].min()==-1.0 else False
    z_max = True if batch[1].max()==1 else False
    z_type = True if batch[0].dtype=="float32" else False
    print("Min of batch test: " + str(z_min))
    print("Max of batch test: " + str(z_max))
    print("Length of batch test: " + str(z_type))
