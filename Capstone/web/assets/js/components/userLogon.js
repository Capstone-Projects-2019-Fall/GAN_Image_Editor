...
logon.registerUser = function (emailId, pwId, pwId2, msgId){
    ...
    var userPassword2 = document.getElementById(pwId2).value;
...
logon.loginUser = function (emailId, pwId, msgId) {
    ...
    ajax({url:"webAPIs/getProfileAPI.jsp", successFn:success, errorId:contentArea});
    ...
    function success(response) {
        ...
        if(response.toString().endsWith("No user logged in.")){ //response.responseText.endsWith("No user logged in.")
            ajax({url:urlString, successFn:showLogon, errorId:contentArea});
        }
        else{
    ...
    function showLogon(response){
        ...
        if(typeof obj === 'string' && obj.includes("User not found") ){
            document.getElementById(contentArea).innerHTML = "Not a valid login. Please try again.";
            return;
        }
        else{
        ...
        myImage.src = obj.userImage;
        myImage.style = "width: 220px;";
        contentElement.appendChild(myImage);
        */
